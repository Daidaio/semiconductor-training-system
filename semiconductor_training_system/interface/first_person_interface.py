...
loader.load('./asml_duv.glb',
  function(gltf){
    try{
      ...
      scene.add(model);
      model.visible=false; // 開始前隱藏內部，先看外殼
...
function createProcObjects(model){
  ...
  if(lzOutNode && _illumBarrelNode2){
    ...
    if(lzBoxNode){
      ...
      console.log('[DBG] Laser_Box bbox centerX='+lzStartX.toFixed(3)+' centerY='+lzBoxY.toFixed(3)
        +' centerZ='+_lzCtr.z.toFixed(3)
        +' minX='+_lzbb2.min.x.toFixed(3)+' maxX='+_lzbb2.max.x.toFixed(3)
        +' minZ='+_lzbb2.min.z.toFixed(3)+' maxZ='+_lzbb2.max.z.toFixed(3));
    }
    // 照明桶中心 X（彎折點 & 垂直段位置）
