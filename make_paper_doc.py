...
def fig_adaptive_flow():
    ...
    # ── 6. 平均分數 ≥ 6.5？ ───────────────────────────────────────────
    dcy3 = 9.07
    diam(CX, dcy3, 1.5, 0.45, '平均分數\n≥ 6.5 分？')
    darr(9.55, dcy3+0.45)

    mode_box(rbox_x, 8.67, 2.3, 0.78,
             '標準模式', '適中題目  ·  少量提示  ·  平衡反饋', shade='#EAF2FB')
    harr(CX+1.5, dcy3, rbox_x, 'YES', lc='#1A7340')

    darr(dcy3-0.45, 8.25, label='NO', lc='#C0392B')

    # ── 7. 平均分數 ≥ 4.0？ ───────────────────────────────────────────
    dcy4 = 7.77
    diam(CX, dcy4, 1.5, 0.45, '平均分數\n≥ 4.0 分？')
    darr(8.25, dcy4+0.45)

    mode_box(rbox_x, 7.37, 2.3, 0.78,
             '鷹架模式', '基礎題目  ·  頻繁提示  ·  詳細反饋', shade='#FEF9E7')
    harr(CX+1.5, dcy4, rbox_x, 'YES', lc='#1A7340')

    darr(dcy4-0.45, 7.0, label='NO', lc='#C0392B')
    ...
    cbox(BX, 4.95, BW, 0.65,
         '故障排除操作  SOP 維修步驟執行',
         'Diagnostic Agent 故障分析  ·  Operation Agent SOP 步驟引導  ·  5 步驟執行')
    darr(4.95, 4.3)
...
def fig_exposure_optical():
    ...
    cbox(BX, 18.0, BW, 0.8,
         '故障情境注入設定  /api/fault',
         '故障類型：contamination · lens_hotspot · stage_error · vacuum_leak · cooling_water')
    darr(18.0, 17.35)
    ...
    harr(CX+1.5, dcy3, rbox_x, 'YES', lc='#1A7340')
    darr(dcy3-0.45, 9.47, label='NO', lc='#C0392B')
    ...
    harr(CX+1.5, dcy4, rbox_x, 'YES', lc='#1A7340')
    darr(dcy4-0.45, 8.12, label='NO', lc='#C0392B')
    ...
    cbox(BX, 6.15, BW, 0.65,
         '故障排除操作  SOP 維修步驟執行',
         'Diagnostic Agent  ·  Operation Agent SOP 步驟引導  ·  5 步驟執行')
    darr(6.15, 5.45)
...
def para(text, indent=True, justify=True):
    ...
    for r in p.runs:
        r.font.name = '新細明體'; r.font.size = Pt(12)
    return p
...
def caption(text):
    ...
    for r in p.runs:
        r.font.name = '新細明體'; r.font.size = Pt(10)
        r.font.italic = True; r.font.color.rgb = RGBColor(0x44,0x44,0x44)
...
def bullet(text, lv=0):
    ...
    r = p.add_run(text)
    r.font.name = '新細明體'; r.font.size = Pt(11)
...
def eq(text, num=''):
    ...
    if num:
        ...
        nr = p2.add_run(num)
        nr.font.name = '新細明體'; nr.font.size = Pt(11); nr.font.italic = True
...
kr = kp.add_run('關鍵詞：')
kr.font.name='新細明體'; kr.font.size=Pt(12); kr.font.bold=True
kp2 = kp.add_run('半導體微影、DUV 虛擬訓練系統、數位孿生、SECOM 資料集、'
                 'Qwen LLM、自適應教學、Three.js、第一人稱訓練環境')
kp2.font.name='新細明體'; kp2.font.size=Pt(12); kp2.font.italic=True
...
para('半導體微影製程是積體電路（IC）製造中技術難度最高、成本最昂貴的關鍵步驟之一。'
     '操作 ASML DUV 微影機台需要具備深厚的光學物理、製程控制與故障診斷知識。'
     '傳統訓練方式高度依賴實體設備時間與資深工程師的現場指導，'
     '對新進工程師與學生而言形成相當高的進入門檻，且實際設備上機機會有限。')
para('本研究提出一套沉浸式虛擬訓練平台，透過第一人稱 3D 互動環境讓受訓者在無需接觸實體設備的情況下，'
     '自由探索機台結構、查看感測器即時讀值、操作虛擬 HMI（人機介面），'
     '並在 AI 導師引導下執行故障診斷與維修程序。'
     '系統以 UCI SECOM 真實製程資料集驅動設備數位孿生，使訓練情境具備真實統計依據。')
para('本系統之主要貢獻包括：'
     '（1）整合 UCI SECOM 真實資料集建立設備數位孿生與感測器即時模擬；'
     '（2）以 WebGL/Three.js 實作具互動 Mesh 的第一人稱 3D 訓練環境；'
     '（3）基於本地端 Qwen LLM 之多代理 AI 導師系統，含自適應四模式教學；'
     '（4）支援七種故障情境診斷與步驟化 SOP 維修引導；'
...
for cell, t in zip(tbl_eq.rows[0].cells, ['模組名稱', '功能說明', '3D 模型 Mesh']):
    ...
    for r in cell.paragraphs[0].runs:
        r.font.bold = True; r.font.size = Pt(10); r.font.name = '新細明體'
eq_rows = [
    ('KrF 準分子雷射光源\n（Excimer Laser）',
    ...
    ('晶圓台\n（Wafer Stage）',
     ...
     'WaferStage_Mesh'),
    ('真空系統\n（Vacuum System）',
     '維持投影鏡組下方真空環境以避免空氣吸收 DUV 光；真空異常為訓練故障情境之一',
     'VacuumPump_Mesh'),
    ('冷卻水系統\n（Cooling Water）',
     '對雷射光源、鏡片及電子元件提供精密溫控冷卻，防止熱漂移影響曝光精度；'
     '冷卻水故障為訓練故障情境之一',
     'CoolingUnit_Mesh'),
]
for name, func, mesh in eq_rows:
    ...
    for c in row:
        for r in c.paragraphs[0].runs:
            r.font.size = Pt(9.5); r.font.name = '新細明體'
...
specs = [
    ...
    ('訓練故障情境',    '7 種：真空異常、冷卻水故障、鏡片熱點、光罩污染、\n晶圓台誤差、劑量漂移、污染異常'),
]
...
# ── 三、系統架構 ──────────────────────────────────────────────────────────────
h('三、系統架構', 1)
para('系統採用六層式垂直架構設計，如圖 1 所示。架構由資料來源層向下流經 AI 導師系統、'
     '後端服務層、使用者介面層、訓練流程層，最終進入評分系統層，'
     '每一層具有明確定義的責任範疇並透過標準化介面與相鄰層溝通。')
doc.add_paragraph()
fig(fig_arch_paper(), w=6.0, cap='圖 1　系統整體架構圖')
...
para('AI 導師系統採用本地端 Qwen 大型語言模型，由三個核心模組組成。'
     ...
     '並持續記錄學習歷程。')
doc.add_paragraph()
fig(fig_adaptive_flow(), w=5.0, cap='圖 2　自適應教學模式判斷流程圖')
...
para('使用者介面以單頁 WebGL 應用程式實作，透過本地 HTTP 伺服器提供服務。'
     ...
     'AI 對話面板（[C] 鍵開啟）提供即時問答與維修 SOP 步驟引導。')
ss('ss_hmi_panel.png', w=5.8, cap='圖 8　HMI 虛擬控制面板（感測器讀值、CDU Map 即時顯示）')
ss('ss_ai_chat.png',   w=2.5, cap='圖 9　AI 對話面板（[C] 鍵開啟）')
...
para('本系統以 Three.js WebGL 技術於瀏覽器端建構全沉浸式 3D 訓練環境，'
     ...
     '涵蓋 11 組可互動零件群組，並支援 7 種製程故障情境的即時注入，'
     '包含真空洩漏、冷卻水異常、鏡片熱點、光罩污染、載台誤差、劑量漂移及焦距漂移。'
     '虛擬 HMI 面板以 HTML5 Canvas 動態繪製，即時顯示 590 個感測器讀值、CDU Map 與 CD 趨勢圖，'
     '並於偵測到故障時觸發警報通知，提供貼近真實操作的訓練情境。')
ss('ss_3d_overview.png',  w=5.8, cap='圖 10　3D 訓練環境全景（第一人稱視角）')
ss('ss_3d_interact.png',  w=5.8, cap='圖 11　靠近零件後出現互動提示（[E] 鍵）')
ss('ss_fault_alarm.png',  w=4.0, cap='圖 12　故障注入後觸發警報通知')

h('2.6　互動零件說明', 2)
para('3D 環境中各零件均設有互動標記，受訓者按下 [E] 鍵後，'
     'AI 導師即依當前故障情境給出對應的檢查提示與引導問題。'
     '下表列出全部可互動零件群組、對應功能及關聯故障情境。')
doc.add_paragraph()

tbl2 = doc.add_table(rows=1, cols=4)
tbl2.style = 'Table Grid'
for cell, t in zip(tbl2.rows[0].cells, ['零件群組', '3D Mesh 代表', '互動功能', '關聯故障情境']):
    cell.text = t
    for r in cell.paragraphs[0].runs:
        r.font.bold=True; r.font.size=Pt(10); r.font.name='新細明體'
mesh_data = [
    ('雷射光源',   'Laser_Box / Laser_Out',         '查看光源強度和穩定性',   '劑量漂移 (dose_drift)'),
    ('照明系統',   'Illum_Barrel / IllumLens_0~5',  '查看光源強度和穩定性',   '焦距漂移 (focus_drift)'),
    ('光罩載台',   'Reticle_Stage / Reticle_Mesh',  '查看光罩載台狀態',       '光罩污染 (contamination)'),
    ('投影鏡組',   'POB_Barrel / Lens_0~9',         '檢查投影鏡組溫度',       '鏡片熱點 (lens_hotspot)'),
    ('晶圓載台',   'Wafer_Chuck / Stage_Base',      '查看晶圓載台位置誤差',   '載台誤差 (stage_error)'),
    ('液浸冷卻',   'Immersion_Hood / Supply',       '檢查冷卻水流量和溫度',   '冷卻水異常 (cooling_water)'),
    ('通風排氣',   'Duct_Top / Vent1 / Vent2',      '檢查真空系統壓力',       '真空洩漏 (vacuum_leak)'),
    ('晶圓傳送手臂','Robot_Arm_Link_Mesh',           '查看晶圓傳送機械手臂',   '—'),
    ('晶圓傳送埠', 'FOUP_Port / FOUP_Door',         '確認晶圓傳送狀態',       '—'),
    ('控制系統',   'Cabinet_Main / Cabinet_Panel',  '查看控制系統狀態',       '—'),
    ('HMI 螢幕',  'HMI_Screen',                    'HMI 控制面板操作',       '—'),
]
for grp_name, mesh, func, fault in mesh_data:
    row = tbl2.add_row().cells
    row[0].text=grp_name; row[1].text=mesh; row[2].text=func; row[3].text=fault
    for c in row:
        for r in c.paragraphs[0].runs:
            r.font.size=Pt(9); r.font.name='新細明體'
caption('表 4　3D 訓練環境可互動零件群組說明')
doc.add_paragraph()

# ── 四、SECOM 流程 ────────────────────────────────────────────────────────────
h('四、SECOM 資料驅動模擬流程', 1)
...
para('DUV 微影製程由晶圓基板準備至下一道製程共包含十二個主要步驟，'
     '圖 3 以學術標準流程圖呈現各步驟的先後順序及關鍵製程參數；'
     '圖 4 放大曝光站之詳細子流程與各量測監控項目；'
     '圖 5 呈現 DUV 曝光站光學路徑，依序經過照明系統（σ 控制）、'
     '光罩（4× 縮小圖案）、投影鏡組（NA 控制），最終投影至晶圓光阻層；'
     '圖 6 則整合訓練互動操作與自適應教學判斷流程，呈現故障注入至 AI 評分驅動之'
     '自適應模式選擇（挑戰/標準/鷹架/補救）、SOP 維修與評分認證的完整訓練互動邏輯。')
doc.add_paragraph()
fig(fig_process_steps(), w=5.2, cap='圖 3　DUV 微影製程步驟流程圖')
doc.add_paragraph()
fig(fig_process(), w=6.4, cap='圖 4　微影製程流程圖（DUV 曝光站詳細）')
doc.add_paragraph()
fig(fig_optical_path(), w=4.0, cap='圖 5　DUV 曝光站光學路徑圖（雷射 → 晶圓）')
doc.add_paragraph()
fig(fig_exposure_optical(), w=5.0, cap='圖 6　訓練互動操作與自適應教學整合流程圖')
...
for name, desc in steps:
    ...
    br = p.add_run(name + '：')
    br.font.name='新細明體'; br.font.size=Pt(11); br.font.bold=True
    nr = p.add_run(desc)
    nr.font.name='新細明體'; nr.font.size=Pt(11)

# ── 六、訓練流程 ──────────────────────────────────────────────────────────────
h('六、訓練系統流程', 1)
para('本系統採整合式訓練設計，理論問答與 3D 實機操作於同一訓練情境中同步進行，'
     '如圖 7 所示。受訓者進入 3D 環境後即可自由操作設備、查看 HMI 面板，'
     '並隨時透過 AI 對話框提問；AI 導師根據輸入內容動態切換理論引導或實作指引，'
     '無需先通過任何獨立的理論測驗。')
doc.add_paragraph()
fig(fig_training(), w=6.2, cap='圖 7　訓練系統流程圖')
...
para('受訓者進入整合訓練情境後，理論學習與實機操作同時展開。'
     ...
     '當受訓者執行設備操作或故障排除時，AI 導師則切換至實作引導模式，'
     '提供 SOP 步驟與即時操作反饋。兩種模式在同一會話中無縫動態切換。')

h('5.2　故障情境與 SOP 維修', 2)
para('系統共提供七種故障情境：真空異常、冷卻水故障、鏡片熱點、光罩污染、'
     '晶圓台位置誤差、劑量漂移與污染異常。每種情境配有明確的觸發 Mesh 群組，'
     '受訓者在 3D 場景中接近對應設備並按 [E] 鍵即可觸發互動。'
     '每種故障配備五步驟 SOP；AI 導師偵測到故障相關關鍵詞時，'
     '自動開啟 SOP 面板提供逐步引導。訓練總評分達 80 分以上即完成並取得能力認證。')

h('5.3　評分系統（ScoringSystem）', 2)
...
terms = [
    ...
    ('SOP（Standard Operating Procedure，標準作業程序）',
     ...
     '本系統提供 7 種故障類型的 SOP，每種包含 5 個執行步驟。'),
    ('PointerLock API',
...
h('8.1　訓練可及性與安全性提升', 2)
para('傳統微影機台操作訓練需佔用昂貴設備機時，且初學者操作失誤可能對設備或晶圓造成實際損壞。'
     '本系統以瀏覽器端 WebGL 環境取代實體機台接觸，受訓者得以在零風險條件下反覆練習七種故障排除情境，'
     '預期可顯著降低訓練成本與實機損耗風險，同時突破時間與地點限制，'
     '提升訓練資源的可及性（accessibility）。')
...
para('系統依據 AI 評分（0–10 分）即時切換挑戰、標準、鷹架、補救四種教學模式，'
     ...
     '並透過蘇格拉底式對話引導受訓者主動建構知識，而非被動接收指令。')

h('8.3　客觀量化能力認證', 2)
para('傳統訓練成效評估多依賴主管主觀判斷或紙筆測驗，難以全面衡量實際操作能力。'
     '本系統以三維度加權評分（診斷準確性 30%、操作正確性 40%、安全合規 30%）'
     '搭配時間效率加分項，自動輸出量化分數與 A–F 五級制等級認定，'
     '預期提供可追蹤、可比較的客觀能力認證依據，'
     '並透過學習歷程紀錄協助管理者掌握整體訓練品質趨勢。')

h('8.4　數位孿生在半導體教育之應用驗證', 2)
para('本研究以 UCI SECOM 真實製程資料集為基礎建立設備數位孿生，'
     '驗證「以公開資料集驅動虛擬訓練環境」之可行性。'
     '預期成果包括：（1）建立感測器雜訊模型與健康分數計算方法，'
     '可移植至其他製程設備；'
     '（2）示範 LLM 本地端部署（Qwen + Ollama）結合製程知識庫的 AI 導師架構，'
     '為後續研究提供可複製的技術基礎；'
     '（3）提出以 Three.js 建構高精度設備 3D 互動模型的工作流程，'
     '供半導體教育領域數位化訓練系統參考。')
doc.add_paragraph()
...
doc.add_page_break()
h('九、結論', 1)
para('本研究提出一套以 AI 導師為核心、數位孿生為基礎之半導體微影設備虛擬訓練系統。'
     '系統透過沉浸式第一人稱 3D 環境，讓受訓者在不接觸實體設備的情況下，'
     '於整合式訓練情境中同步進行理論問答與實機操作模擬，AI 導師動態切換引導模式。'
     'UCI SECOM 真實製程資料集的整合，使設備數位孿生的感測器輸出具備真實統計依據；'
     '多代理 AI 架構（Diagnostic Agent、Operation Agent、Adaptive Teaching）'
     '實現個人化、自適應的四模式導師引導；'
     '三維度量化評分系統則提供客觀、可量化的能力認證機制。'
     '後續研究方向包括：擴充故障情境種類、引入多人協同訓練模式，'
     '以及將訓練環境延伸至光罩搬運與計量量測（Metrology）完整工作流程。')
...
for ref in refs:
    ...
    for r in rp.runs:
        r.font.size=Pt(11); r.font.name='新細明體'
